        ...
        if (cep->psize == 4 && cep->vsize == 2) {
            ...
            if (cmd == EQ_PARAM_CUR_PRESET && arg >= 0 && arg < gNumPresets) {
                ...
                for (i = 0; i < NUM_BANDS; i++) {
                    mBand[i] = gEqualizerPresets[arg].bandConfigs[i] / 100.0;
                }
...
int32_t EffectEqualizer::process(audio_buffer_t *in, audio_buffer_t *out)
{
	...
	for (uint32_t i = 0; i < in->frameCount; i ++) {
		...
		if (formatFloatModeInt32Mode == 0) {
			...
			mPowerSquaredR += pow(tmpL, 2);
		}
		else if (formatFloatModeInt32Mode == 1) {
			tmpL = readPcmFloat(in, i * 2);
			tmpR = readPcmFloat(in, i * 2 + 1);
		...
		if (formatFloatModeInt32Mode == 0) {
			...
			writePcmFloat(out, i * 2, tmpL);
			writePcmFloat(out, i * 2 + 1, tmpR);
		}
		else if (formatFloatModeInt32Mode == 2) {
			out->s32[i * 2] = (int32_t)tmpL;
			out->s32[i * 2 + 1] = (int32_t)tmpR;
		}
    }
