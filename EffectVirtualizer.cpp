...
int32_t EffectVirtualizer::process(audio_buffer_t* in, audio_buffer_t* out)
{
	for (uint32_t i = 0; i < in->frameCount; i ++) {
		...
		if (formatFloatModeInt32Mode == 0) {
			...
			dryL = readPcmFloat(in, i * 2);
			dryR = readPcmFloat(in, i * 2 + 1);
			dataL = dryL;
			dataR = dryR;
		}
		else if (formatFloatModeInt32Mode == 2) {
			dryL = (double)in->s32[i * 2];
			dryR = (double)in->s32[i * 2 + 1];
			dataL = dryL;
			dataR = dryR;
		}
        ...
        if (formatFloatModeInt32Mode == 0) {
			...
			writePcmFloat(out, i * 2, center + side);
			writePcmFloat(out, i * 2 + 1, center - side);
		}
        else if (formatFloatModeInt32Mode == 2) {
			/* Center channel. */
			double center = (dataL + dataR) / 2;
			...
			out->s32[i * 2] = (int32_t)(center + side);
			out->s32[i * 2 + 1] = (int32_t)(center - side);
		}
    }
